...
HXLINE(  27)			::Sys_obj::print(::Markdown_obj::markdownToHtml(source));
...
HXLINE(  49)			return ::Markdown_obj::renderHtml(document->parseLines(lines));
            		} catch( ::Dynamic _hx_e) {
            			if (_hx_e.IsClass<  ::Dynamic >() ){
            				HX_STACK_BEGIN_CATCH
            				 ::Dynamic _g = _hx_e;
...
HXLINE(  51)				return ((HX_("<pre>",f7,8c,69,d6) + ::Std_obj::string(_g)) + HX_("</pre>",ae,a8,9d,57));
            			}
            			else {
            				HX_STACK_DO_THROW(_hx_e);
            			}
