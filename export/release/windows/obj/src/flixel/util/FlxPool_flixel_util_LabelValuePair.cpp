// Generated by Haxe 4.3.6
#include <hxcpp.h>

#ifndef INCLUDED_Type
#include <Type.h>
#endif
#ifndef INCLUDED_flixel_util_FlxPool_flixel_util_LabelValuePair
#include <flixel/util/FlxPool_flixel_util_LabelValuePair.h>
#endif
#ifndef INCLUDED_flixel_util_IFlxDestroyable
#include <flixel/util/IFlxDestroyable.h>
#endif
#ifndef INCLUDED_flixel_util_IFlxPool
#include <flixel/util/IFlxPool.h>
#endif
#ifndef INCLUDED_flixel_util_LabelValuePair
#include <flixel/util/LabelValuePair.h>
#endif

HX_DEFINE_STACK_FRAME(_hx_pos_a10ee8e234039862_12_new,"flixel.util.FlxPool_flixel_util_LabelValuePair","new",0x9e8bdd4e,"flixel.util.FlxPool_flixel_util_LabelValuePair.new","flixel/util/FlxPool.hx",12,0xdd4de86a)
HX_LOCAL_STACK_FRAME(_hx_pos_a10ee8e234039862_35_get,"flixel.util.FlxPool_flixel_util_LabelValuePair","get",0x9e868d84,"flixel.util.FlxPool_flixel_util_LabelValuePair.get","flixel/util/FlxPool.hx",35,0xdd4de86a)
HX_LOCAL_STACK_FRAME(_hx_pos_a10ee8e234039862_49_put,"flixel.util.FlxPool_flixel_util_LabelValuePair","put",0x9e8d6fbd,"flixel.util.FlxPool_flixel_util_LabelValuePair.put","flixel/util/FlxPool.hx",49,0xdd4de86a)
HX_LOCAL_STACK_FRAME(_hx_pos_a10ee8e234039862_65_putUnsafe,"flixel.util.FlxPool_flixel_util_LabelValuePair","putUnsafe",0xa6da53c3,"flixel.util.FlxPool_flixel_util_LabelValuePair.putUnsafe","flixel/util/FlxPool.hx",65,0xdd4de86a)
HX_LOCAL_STACK_FRAME(_hx_pos_a10ee8e234039862_76_preAllocate,"flixel.util.FlxPool_flixel_util_LabelValuePair","preAllocate",0xa9e6f4ee,"flixel.util.FlxPool_flixel_util_LabelValuePair.preAllocate","flixel/util/FlxPool.hx",76,0xdd4de86a)
HX_LOCAL_STACK_FRAME(_hx_pos_a10ee8e234039862_86_clear,"flixel.util.FlxPool_flixel_util_LabelValuePair","clear",0xee6315fb,"flixel.util.FlxPool_flixel_util_LabelValuePair.clear","flixel/util/FlxPool.hx",86,0xdd4de86a)
HX_LOCAL_STACK_FRAME(_hx_pos_a10ee8e234039862_95_get_length,"flixel.util.FlxPool_flixel_util_LabelValuePair","get_length",0x32969181,"flixel.util.FlxPool_flixel_util_LabelValuePair.get_length","flixel/util/FlxPool.hx",95,0xdd4de86a)
namespace flixel{
namespace util{

void FlxPool_flixel_util_LabelValuePair_obj::__construct(::hx::Class classObj){
            	HX_STACKFRAME(&_hx_pos_a10ee8e234039862_12_new)
HXLINE(  27)		this->_count = 0;
HXLINE(  16)		this->_pool = ::Array_obj< ::Dynamic>::__new(0);
HXLINE(  31)		this->_class = classObj;
            	}

Dynamic FlxPool_flixel_util_LabelValuePair_obj::__CreateEmpty() { return new FlxPool_flixel_util_LabelValuePair_obj; }

void *FlxPool_flixel_util_LabelValuePair_obj::_hx_vtable = 0;

Dynamic FlxPool_flixel_util_LabelValuePair_obj::__Create(::hx::DynamicArray inArgs)
{
	::hx::ObjectPtr< FlxPool_flixel_util_LabelValuePair_obj > _hx_result = new FlxPool_flixel_util_LabelValuePair_obj();
	_hx_result->__construct(inArgs[0]);
	return _hx_result;
}

bool FlxPool_flixel_util_LabelValuePair_obj::_hx_isInstanceOf(int inClassId) {
	return inClassId==(int)0x00000001 || inClassId==(int)0x71f8c820;
}

static ::flixel::util::IFlxPool_obj _hx_flixel_util_FlxPool_flixel_util_LabelValuePair__hx_flixel_util_IFlxPool= {
	( void (::hx::Object::*)(int))&::flixel::util::FlxPool_flixel_util_LabelValuePair_obj::preAllocate,
	( ::cpp::VirtualArray (::hx::Object::*)())&::flixel::util::FlxPool_flixel_util_LabelValuePair_obj::clear_36f85899,
};

::cpp::VirtualArray FlxPool_flixel_util_LabelValuePair_obj::clear_36f85899() {
			return clear();
}
void *FlxPool_flixel_util_LabelValuePair_obj::_hx_getInterface(int inHash) {
	switch(inHash) {
		case (int)0x5a90a383: return &_hx_flixel_util_FlxPool_flixel_util_LabelValuePair__hx_flixel_util_IFlxPool;
	}
	#ifdef HXCPP_SCRIPTABLE
	return super::_hx_getInterface(inHash);
	#else
	return 0;
	#endif
}

 ::flixel::util::LabelValuePair FlxPool_flixel_util_LabelValuePair_obj::get(){
            	HX_STACKFRAME(&_hx_pos_a10ee8e234039862_35_get)
HXLINE(  37)		if ((this->_count == 0)) {
HXLINE(  40)			return ( ( ::flixel::util::LabelValuePair)(::Type_obj::createInstance(this->_class,::cpp::VirtualArray_obj::__new(0))) );
            		}
HXLINE(  43)		return this->_pool->__get(--this->_count).StaticCast<  ::flixel::util::LabelValuePair >();
            	}


HX_DEFINE_DYNAMIC_FUNC0(FlxPool_flixel_util_LabelValuePair_obj,get,return )

void FlxPool_flixel_util_LabelValuePair_obj::put( ::flixel::util::LabelValuePair obj){
            	HX_STACKFRAME(&_hx_pos_a10ee8e234039862_49_put)
HXDLIN(  49)		if (::hx::IsNotNull( obj )) {
HXLINE(  51)			int i = this->_pool->indexOf(obj,null());
HXLINE(  53)			bool _hx_tmp;
HXDLIN(  53)			if ((i != -1)) {
HXLINE(  53)				_hx_tmp = (i >= this->_count);
            			}
            			else {
HXLINE(  53)				_hx_tmp = true;
            			}
HXDLIN(  53)			if (_hx_tmp) {
HXLINE(  57)				{
HXLINE(  57)					obj->label = null();
HXDLIN(  57)					obj->value = null();
            				}
HXLINE(  58)				this->_pool[this->_count++] = obj;
            			}
            		}
            	}


HX_DEFINE_DYNAMIC_FUNC1(FlxPool_flixel_util_LabelValuePair_obj,put,(void))

void FlxPool_flixel_util_LabelValuePair_obj::putUnsafe( ::flixel::util::LabelValuePair obj){
            	HX_STACKFRAME(&_hx_pos_a10ee8e234039862_65_putUnsafe)
HXDLIN(  65)		if (::hx::IsNotNull( obj )) {
HXLINE(  69)			{
HXLINE(  69)				obj->label = null();
HXDLIN(  69)				obj->value = null();
            			}
HXLINE(  70)			this->_pool[this->_count++] = obj;
            		}
            	}


HX_DEFINE_DYNAMIC_FUNC1(FlxPool_flixel_util_LabelValuePair_obj,putUnsafe,(void))

void FlxPool_flixel_util_LabelValuePair_obj::preAllocate(int numObjects){
            	HX_STACKFRAME(&_hx_pos_a10ee8e234039862_76_preAllocate)
HXDLIN(  76)		while(true){
HXDLIN(  76)			numObjects = (numObjects - 1);
HXDLIN(  76)			if (!(((numObjects + 1) > 0))) {
HXDLIN(  76)				goto _hx_goto_4;
            			}
HXLINE(  81)			this->_pool[this->_count++] = ( ( ::flixel::util::LabelValuePair)(::Type_obj::createInstance(this->_class,::cpp::VirtualArray_obj::__new(0))) );
            		}
            		_hx_goto_4:;
            	}


HX_DEFINE_DYNAMIC_FUNC1(FlxPool_flixel_util_LabelValuePair_obj,preAllocate,(void))

::Array< ::Dynamic> FlxPool_flixel_util_LabelValuePair_obj::clear(){
            	HX_STACKFRAME(&_hx_pos_a10ee8e234039862_86_clear)
HXLINE(  87)		this->_count = 0;
HXLINE(  88)		::Array< ::Dynamic> oldPool = this->_pool;
HXLINE(  89)		this->_pool = ::Array_obj< ::Dynamic>::__new(0);
HXLINE(  90)		return oldPool;
            	}


HX_DEFINE_DYNAMIC_FUNC0(FlxPool_flixel_util_LabelValuePair_obj,clear,return )

int FlxPool_flixel_util_LabelValuePair_obj::get_length(){
            	HX_STACKFRAME(&_hx_pos_a10ee8e234039862_95_get_length)
HXDLIN(  95)		return this->_count;
            	}


HX_DEFINE_DYNAMIC_FUNC0(FlxPool_flixel_util_LabelValuePair_obj,get_length,return )


::hx::ObjectPtr< FlxPool_flixel_util_LabelValuePair_obj > FlxPool_flixel_util_LabelValuePair_obj::__new(::hx::Class classObj) {
	::hx::ObjectPtr< FlxPool_flixel_util_LabelValuePair_obj > __this = new FlxPool_flixel_util_LabelValuePair_obj();
	__this->__construct(classObj);
	return __this;
}

::hx::ObjectPtr< FlxPool_flixel_util_LabelValuePair_obj > FlxPool_flixel_util_LabelValuePair_obj::__alloc(::hx::Ctx *_hx_ctx,::hx::Class classObj) {
	FlxPool_flixel_util_LabelValuePair_obj *__this = (FlxPool_flixel_util_LabelValuePair_obj*)(::hx::Ctx::alloc(_hx_ctx, sizeof(FlxPool_flixel_util_LabelValuePair_obj), true, "flixel.util.FlxPool_flixel_util_LabelValuePair"));
	*(void **)__this = FlxPool_flixel_util_LabelValuePair_obj::_hx_vtable;
	__this->__construct(classObj);
	return __this;
}

FlxPool_flixel_util_LabelValuePair_obj::FlxPool_flixel_util_LabelValuePair_obj()
{
}

void FlxPool_flixel_util_LabelValuePair_obj::__Mark(HX_MARK_PARAMS)
{
	HX_MARK_BEGIN_CLASS(FlxPool_flixel_util_LabelValuePair);
	HX_MARK_MEMBER_NAME(_pool,"_pool");
	HX_MARK_MEMBER_NAME(_class,"_class");
	HX_MARK_MEMBER_NAME(_count,"_count");
	HX_MARK_END_CLASS();
}

void FlxPool_flixel_util_LabelValuePair_obj::__Visit(HX_VISIT_PARAMS)
{
	HX_VISIT_MEMBER_NAME(_pool,"_pool");
	HX_VISIT_MEMBER_NAME(_class,"_class");
	HX_VISIT_MEMBER_NAME(_count,"_count");
}

::hx::Val FlxPool_flixel_util_LabelValuePair_obj::__Field(const ::String &inName,::hx::PropertyAccess inCallProp)
{
	switch(inName.length) {
	case 3:
		if (HX_FIELD_EQ(inName,"get") ) { return ::hx::Val( get_dyn() ); }
		if (HX_FIELD_EQ(inName,"put") ) { return ::hx::Val( put_dyn() ); }
		break;
	case 5:
		if (HX_FIELD_EQ(inName,"_pool") ) { return ::hx::Val( _pool ); }
		if (HX_FIELD_EQ(inName,"clear") ) { return ::hx::Val( clear_dyn() ); }
		break;
	case 6:
		if (HX_FIELD_EQ(inName,"length") ) { if (inCallProp == ::hx::paccAlways) return ::hx::Val( get_length() ); }
		if (HX_FIELD_EQ(inName,"_class") ) { return ::hx::Val( _class ); }
		if (HX_FIELD_EQ(inName,"_count") ) { return ::hx::Val( _count ); }
		break;
	case 9:
		if (HX_FIELD_EQ(inName,"putUnsafe") ) { return ::hx::Val( putUnsafe_dyn() ); }
		break;
	case 10:
		if (HX_FIELD_EQ(inName,"get_length") ) { return ::hx::Val( get_length_dyn() ); }
		break;
	case 11:
		if (HX_FIELD_EQ(inName,"preAllocate") ) { return ::hx::Val( preAllocate_dyn() ); }
	}
	return super::__Field(inName,inCallProp);
}

::hx::Val FlxPool_flixel_util_LabelValuePair_obj::__SetField(const ::String &inName,const ::hx::Val &inValue,::hx::PropertyAccess inCallProp)
{
	switch(inName.length) {
	case 5:
		if (HX_FIELD_EQ(inName,"_pool") ) { _pool=inValue.Cast< ::Array< ::Dynamic> >(); return inValue; }
		break;
	case 6:
		if (HX_FIELD_EQ(inName,"_class") ) { _class=inValue.Cast< ::hx::Class >(); return inValue; }
		if (HX_FIELD_EQ(inName,"_count") ) { _count=inValue.Cast< int >(); return inValue; }
	}
	return super::__SetField(inName,inValue,inCallProp);
}

void FlxPool_flixel_util_LabelValuePair_obj::__GetFields(Array< ::String> &outFields)
{
	outFields->push(HX_("length",e6,94,07,9f));
	outFields->push(HX_("_pool",bb,9c,6d,fd));
	outFields->push(HX_("_class",79,bf,3f,44));
	outFields->push(HX_("_count",10,8c,4a,46));
	super::__GetFields(outFields);
};

#ifdef HXCPP_SCRIPTABLE
static ::hx::StorageInfo FlxPool_flixel_util_LabelValuePair_obj_sMemberStorageInfo[] = {
	{::hx::fsObject /* ::Array< ::Dynamic> */ ,(int)offsetof(FlxPool_flixel_util_LabelValuePair_obj,_pool),HX_("_pool",bb,9c,6d,fd)},
	{::hx::fsObject /* ::hx::Class */ ,(int)offsetof(FlxPool_flixel_util_LabelValuePair_obj,_class),HX_("_class",79,bf,3f,44)},
	{::hx::fsInt,(int)offsetof(FlxPool_flixel_util_LabelValuePair_obj,_count),HX_("_count",10,8c,4a,46)},
	{ ::hx::fsUnknown, 0, null()}
};
static ::hx::StaticInfo *FlxPool_flixel_util_LabelValuePair_obj_sStaticStorageInfo = 0;
#endif

static ::String FlxPool_flixel_util_LabelValuePair_obj_sMemberFields[] = {
	HX_("_pool",bb,9c,6d,fd),
	HX_("_class",79,bf,3f,44),
	HX_("_count",10,8c,4a,46),
	HX_("get",96,80,4e,00),
	HX_("put",cf,62,55,00),
	HX_("putUnsafe",55,e0,1c,e4),
	HX_("preAllocate",00,4a,53,a6),
	HX_("clear",8d,71,5b,48),
	HX_("get_length",af,04,8f,8f),
	::String(null()) };

::hx::Class FlxPool_flixel_util_LabelValuePair_obj::__mClass;

void FlxPool_flixel_util_LabelValuePair_obj::__register()
{
	FlxPool_flixel_util_LabelValuePair_obj _hx_dummy;
	FlxPool_flixel_util_LabelValuePair_obj::_hx_vtable = *(void **)&_hx_dummy;
	::hx::Static(__mClass) = new ::hx::Class_obj();
	__mClass->mName = HX_("flixel.util.FlxPool_flixel_util_LabelValuePair",5c,de,ab,60);
	__mClass->mSuper = &super::__SGetClass();
	__mClass->mConstructEmpty = &__CreateEmpty;
	__mClass->mConstructArgs = &__Create;
	__mClass->mGetStaticField = &::hx::Class_obj::GetNoStaticField;
	__mClass->mSetStaticField = &::hx::Class_obj::SetNoStaticField;
	__mClass->mStatics = ::hx::Class_obj::dupFunctions(0 /* sStaticFields */);
	__mClass->mMembers = ::hx::Class_obj::dupFunctions(FlxPool_flixel_util_LabelValuePair_obj_sMemberFields);
	__mClass->mCanCast = ::hx::TCanCast< FlxPool_flixel_util_LabelValuePair_obj >;
#ifdef HXCPP_SCRIPTABLE
	__mClass->mMemberStorageInfo = FlxPool_flixel_util_LabelValuePair_obj_sMemberStorageInfo;
#endif
#ifdef HXCPP_SCRIPTABLE
	__mClass->mStaticStorageInfo = FlxPool_flixel_util_LabelValuePair_obj_sStaticStorageInfo;
#endif
	::hx::_hx_RegisterClass(__mClass->mName, __mClass);
}

} // end namespace flixel
} // end namespace util
