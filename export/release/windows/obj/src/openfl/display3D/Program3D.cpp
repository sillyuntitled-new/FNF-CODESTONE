...
HXLINE( 433)		::String vertex = (HX_("#ifdef GL_ES\r\n\t\t\t#ifdef GL_FRAGMENT_PRECISION_HIGH\r\n\t\t\tprecision highp float;\r\n\t\t\t#else\r\n\t\t\tprecision mediump float;\r\n\t\t\t#endif\r\n\t\t\t#endif\r\n\t\t\t",95,e0,55,cb) + vertexSource);
HXLINE( 434)		::String fragment = (HX_("#ifdef GL_ES\r\n\t\t\t#ifdef GL_FRAGMENT_PRECISION_HIGH\r\n\t\t\tprecision highp float;\r\n\t\t\t#else\r\n\t\t\tprecision mediump float;\r\n\t\t\t#endif\r\n\t\t\t#endif\r\n\t\t\t",95,e0,55,cb) + fragmentSource);
...
HXDLIN( 460)				_g = (_g + 1);
HXLINE( 462)				int index1 = gl->getUniformLocation(this->_hx___glProgram,name);
HXDLIN( 462)				index = index1;
HXLINE( 463)				this->_hx___glslSamplerNames[index1] = name;
            			}
            		}
HXLINE( 466)		{
...
HXLINE( 508)				uniform->name = ( (::String)(info->__Field(HX_("name",4b,72,ff,48),::hx::paccDynamic)) );
HXLINE( 509)				uniform->size = ( (int)(info->__Field(HX_("size",c1,a0,53,4c),::hx::paccDynamic)) );
HXLINE( 510)				uniform->type = ( (int)(info->__Field(HX_("type",ba,f2,08,4d),::hx::paccDynamic)) );
...
HXLINE( 533)				uniform->regCount = (uniform->regCount * uniform->size);
...
HXLINE( 560)										this->_hx___agalSamplerUsageMask = (this->_hx___agalSamplerUsageMask | (1 << (uniform->regIndex + (_g2 - 1))));
            									}
            								}
            							}
            							else {
...
HXLINE( 563)									_hx_tmp1 = ::StringTools_obj::endsWith(uniform->name,HX_("_alpha",9f,ee,7d,1d));
            								}
            								else {
...
HXLINE( 569)										_hx_tmp2 = ::StringTools_obj::endsWith(uniform->name,HX_("_alphaEnabled",a2,df,12,f4));
            									}
            									else {
...
HXLINE( 662)					if ((val->regCount == 1)) {
HXLINE( 664)						gl->uniform1i(val->location,val->regIndex);
            					}
            					else {
...
HXLINE( 674)					if ((val1->regCount == 1)) {
HXLINE( 676)						gl->uniform1i(val1->location,val1->regIndex);
            					}
            					else {
...
HXLINE( 867)			message = (HX_("Error compiling vertex shader",39,c9,c1,b2) + (HX_("\n",0a,00,00,00) + gl->getShaderInfoLog(this->_hx___glVertexShader)));
...
HXLINE( 879)			message1 = (HX_("Error compiling fragment shader",4d,d0,be,e3) + (HX_("\n",0a,00,00,00) + gl->getShaderInfoLog(this->_hx___glFragmentShader)));
...
HXLINE( 921)			message2 = (HX_("Unable to initialize the shader program",f0,86,8b,39) + (HX_("\n",0a,00,00,00) + gl->getProgramInfoLog(this->_hx___glProgram)));
