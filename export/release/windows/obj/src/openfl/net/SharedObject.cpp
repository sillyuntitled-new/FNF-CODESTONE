...
HXDLIN(1010)		try {
            			HX_STACK_CATCHABLE( ::Dynamic, 0);
HXLINE(1013)			return ::haxe::io::Bytes_obj::ofString(::haxe::Serializer_obj::run(this->data),null())->length;
            		} catch( ::Dynamic _hx_e) {
            			if (_hx_e.IsClass<  ::Dynamic >() ){
            				HX_STACK_BEGIN_CATCH
            				 ::Dynamic _g = _hx_e;
...
HXLINE( 901)					_g = (_g + 1);
HXLINE( 903)					name = (name + ((HX_("#",23,00,00,00) + split->__get((_g - 1))) + HX_("/",2f,00,00,00)));
            				}
            			}
...
HXLINE(1003)			( ( ::openfl::net::SharedObject)(sharedObject->__Field(HX_("next",f3,84,02,49),::hx::paccDynamic)()) )->flush(null());
            		}
            	}
